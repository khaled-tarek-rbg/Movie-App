...
@section('comment-form')
    <form action="{{ route('anime.season.comment') }}" method="POST">

        @csrf

        <input type="text" placeholder="write comment" class="ms-2 rounded border-0 h-100" name="title">

        <input type="hidden" value="{{ $season->id }}" name="anime_season">
...
@section('content')
    <div class="col-lg-6 mx-auto film-center" style="background-color: #0C0F21 ; padding-top: 70px;">
        <div class="row position-relative "
            ...
            <div class="content py-4 normal-w d-none">
                ...
                <ul class="unstyled d-flex ">
                    ...
                    <li><a href="#"> {{ $season->anime->name }} {{ $season->season_number }} </a></li>
                </ul>


                <h4>{{ $season->anime->getTranslation('name', 'en') }} {{ $season->season_number }}</h4>


                <div class="film-info py-2">
                    <ul class="m-0 p-0">
                        @if ($season->anime->getTranslation('name', 'ar') == true)
                            <li><span>Name in Arabic</span>
                                <p class="m-0"> {{ $season->anime->getTranslation('name', 'ar') }} </p>
                            </li>
                        @endif
                        <li><span>Categorization</span>
                            <p class="m-0">
                                @foreach ($season->anime->categories as $item)
                                    {{ $item->name }}
                                @endforeach
                            </p>
                        </li>
                ...
                <div class="row d-flex">
                    ...
                    <div class="col-lg-7 large-w-content">
                        <ul class="unstyled d-flex ">
                            ...
                            <li><a href="#">Series</a></li>
                            <li><a href="#">English Series</a></li>
                            <li><a href="#"> serie {{ $season->anime->getTranslation('name', 'en') }}
                                    {{ $season->season_number }}
                                </a></li>
                        </ul>


                        <h4>{{ $season->anime->getTranslation('name', 'en') }} ( {{ $season->season_production_year }} )
                        </h4>
        ...
        @if ($season->anime->description !== null)
            <div class="row"
        ...
        <div class="row py-3 " style="background-color: #1A1D2E">
            ...
            @foreach ($season->eposides as $item)
                <div class="col-lg-6 mt-2">
                    <a class='text-white d-block  w-100 ' href='{{ route('animes.eposide.show', ['id' => $item->id]) }}'
                        style='background-color: #393D51 ; border-radius: 17px ; padding: 12px '>eposide
                        {{ $item->eposide_number }}</a>
                </div>
            @endforeach
