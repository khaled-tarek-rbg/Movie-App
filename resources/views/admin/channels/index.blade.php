                    ...
                    <tbody>
                        ...
                        @foreach ($channels as $type)
                            <tr>
                                ...
                                <td>{{ $type->name }}</td>
                                <td>
                                    <img src="{{ Storage::url($type->poster) }}" alt="" width="100px">
                                </td>
                                <td>{{ $type->channelCountry->channel_country }}</td>
                                <td>{{ $type->channelType->name }}</td>

                                <td><a class="btn btn-link " href="{{ route('channels.admin.edit', ['id' => $type->id]) }}"><i
                                            class="fas fa-edit"></i></a>
                                </td>
                                <td><a class="btn btn-link "
                                        href="{{ route('channels.admin.show', ['id' => $type->id]) }}"><i
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-body">
                                            ...
                                            <div class="modal-footer">
                                                <form action="{{ route('channels.admin.delete', ['id' => $type->id]) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
