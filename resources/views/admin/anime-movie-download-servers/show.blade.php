    ...
    <div class="col-lg-12">
        <div class="admin-dashboard-content p-3  overflow-auto rounded" style="background-color: #262B40">
            <table class="table  ">
                ...
                @if (count($anime->AnimeDownloadsServers) == 0)
                ...
                @else
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>
                                @foreach ($anime->AnimeDownloadsServers as $item)
                            ...
                            <td>
                                @foreach ($anime->AnimeDownloadsServers as $item)
                                    <a class="d-block"
                                        href="{{ route('movies.downloadservers.edit.animes', ['id' => $item->id]) }}">
                            ...
                            <td>
                                @foreach ($anime->AnimeDownloadsServers as $item)
                                    <button type="button" class="btn btn-link d-block p-0 m-0 " data-bs-toggle="modal"
                                        ...
                                        <div class="modal-dialog  ">
                                            <div class="modal-content ">
                                                <div class="modal-body  ">
                                                    Are you sure you want to delete {{ $item->AnimeMovieDownload->name }}
                                                    download server
                                                    <div class="modal-footer">
                                                        <form
                                                            action="{{ route('movies.downloadservers.delete.animes', ['id' => $item->id]) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
