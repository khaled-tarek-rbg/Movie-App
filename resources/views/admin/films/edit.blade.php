    ...
    <div class="col-lg-12">
        <div class="admin-dashboard-content  rounded">
            <form action="{{ route('movies.update', ['id' => $movie->id]) }}" method="POST" enctype="multipart/form-data">
                ...
                <input type="text" name="movie_name_ar" class="form-control mt-2" placeholder="movie name in Arabic"
                ...
                @livewire('movie-type-edit', [$movie])

                <div class="all-servers d-flex w-100   justify-content-between  align-items-center"
                    style="padding: 10px 120px">
                    <a href="{{ route('server.watch.show', ['id' => $movie->id]) }}"
                        class=" text-decoration-none m-0 text-white">movie watch servers
                        <i class="icon-animation fas fa-arrow-circle-right ms-2"></i>
                    </a>


                    <a href="{{ route('server.download.show', ['id' => $movie->id]) }}"
